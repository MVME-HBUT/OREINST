        ...
        if self.training:
            ...
            if len(pred_instances) == 0:
                ...
                if self.boxinst_enabled:
                    ...
                    ######################################################################
                    """
                    image_color_similarity_1 = torch.cat([x.image_color_similarity_1 for x in gt_instances])
                    image_color_similarity_1 = image_color_similarity_1[gt_inds].to(dtype=mask_feats.dtype)

                    # image_color_similarity_2 = torch.cat([x.image_color_similarity_2 for x in gt_instances])
                    # image_color_similarity_2 = image_color_similarity_2[gt_inds].to(dtype=mask_feats.dtype)
                    ...
                    weights_1 = (image_color_similarity_1 >= self.pairwise_color_thresh).float() * gt_bitmasks.float()
                    loss_pairwise_1 = (pairwise_losses * weights_1).sum() / weights_1.sum().clamp(min=1.0)

                    # weights_2 = (image_color_similarity_2 >= self.pairwise_color_thresh).float() * gt_bitmasks.float()
                    # loss_pairwise_2 = (pairwise_losses * weights_2).sum() / weights_2.sum().clamp(min=1.0)

                    warmup_factor = min(self._iter.item() / float(self._warmup_iters), 1.0)
                    # loss_pairwise = loss_pairwise * warmup_factor
                    loss_pairwise_1 = loss_pairwise_1 * warmup_factor
                    # loss_pairwise_2 = loss_pairwise_2 * warmup_factor
                    ...
                    losses.update({
                        "loss_prj": loss_prj_term,
                        "loss_pairwise_1": loss_pairwise_1,
                        # "loss_pairwise_2": loss_pairwise_2,
                    })
                else:
