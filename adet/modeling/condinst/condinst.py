...
def get_images_color_similarity(images, image_masks, kernel_size, dilation):
    ...
    # F = torch.norm(diff, p=2, dim=1)
    # FF = pow(F, 2) 
    # similarity = torch.exp(-FF* 0.5)
...
class CondInst(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        PRECOLOR_THRESH
        """
        self.pairwise_precolor_thresh = cfg.MODEL.BOXINST.PAIRWISE.PRECOLOR_THRESH
    ...
    def forward(self, batched_inputs):
        ...
        time6 = time.time()
        # print("backbone:" + str(float(time2-time1)*100000))
        # print("mask:" + str(float(time4-time3)*100000))
        # print("proposals:" + str(float(time6-time5)*100000))
        # print("test12:" + str(float(timetest2-timetest1)*100000))
        # print("test34:" + str(float(timetest4-timetest3)*100000))
        # print("test3_1_3:" + str(float(timetest3_1-timetest3)*100000))
        # print("test3_2_3_1:" + str(float(timetest3_2-timetest3_1)*100000))
        # print("test4_3_2:" + str(float(timetest4-timetest3_2)*100000))
        # self.normtime = self.normtime + (timetest4-timetest3_2)
        # print("allnormtime: " + str(self.normtime))
        # # print("allnormtime: " + str(timetest4-timetest3_2))
        # print("alltime:" + str(float(time2-time1 + time4-time3 + time6-time5 + timetest2-timetest1 + timetest4-timetest3)*100000))
        if self.training:
    ...
    def add_bitmasks_from_boxes(self, instances, images, image_masks, im_h, im_w):
        ...
        for im_i, per_im_gt_inst in enumerate(instances):
            ...
            images_color_similarity_lab = get_images_color_similarity(
                images_lab, image_masks[im_i],
                self.pairwise_size, self.pairwise_dilation
            )
            # print("LAB") 
            # print(images_color_similarity_lab)
            """
            HSV
            """
            # images_hsv = color.rgb2hsv(downsampled_images[im_i].byte().permute(1, 2, 0).cpu().numpy())
            # images_hsv = torch.as_tensor(images_hsv, device=downsampled_images.device, dtype=torch.float32)
            # images_hsv = images_hsv.permute(2, 0, 1)[None]
            # images_color_similarity_hsv = get_images_color_similarity(
            #     images_hsv, image_masks[im_i],
            #     self.pairwise_size, self.pairwise_dilation
            # )
            """
            XYZ
            """
            # images_xyz = color.rgb2xyz(downsampled_images[im_i].byte().permute(1, 2, 0).cpu().numpy())
            # images_xyz = torch.as_tensor(images_xyz, device=downsampled_images.device, dtype=torch.float32)
            # images_xyz = images_xyz.permute(2, 0, 1)[None]
            # images_color_similarity_xyz = get_images_color_similarity(
            #     images_xyz, image_masks[im_i],
            #     self.pairwise_size, self.pairwise_dilation
            # )
            # print("XYZ") 
            # print(images_color_similarity_xyz)
            """
            GRAY
            """
            # images_gray = color.rgb2gray(downsampled_images[im_i].byte().permute(1, 2, 0).cpu().numpy())
            # images_gray = numpy.array([images_gray,images_gray,images_gray])
            # images_gray = images_gray.transpose(1, 2, 0)
            # images_gray = torch.as_tensor(images_gray, device=downsampled_images.device, dtype=torch.float32)
            # images_gray = images_gray.permute(2, 0, 1)[None]
            # images_color_similarity_gray = get_images_color_similarity(
            #     images_gray, image_masks[im_i],
            #     self.pairwise_size, self.pairwise_dilation
            # )
            """
            LBP
            """
            images_gray = color.rgb2gray(downsampled_images[im_i].byte().permute(1, 2, 0).cpu().numpy())
            images_lbp = skft.local_binary_pattern(images_gray, 8, 1, method='uniform')
            images_lbp = numpy.array([images_lbp, images_lbp, images_lbp])
            images_lbp = images_lbp.transpose(1, 2, 0)
            images_lbp = torch.as_tensor(images_lbp, device=downsampled_images.device, dtype=torch.float32)
            images_lbp = images_lbp.permute(2, 0, 1)[None]
            images_color_similarity_lbp = get_images_color_similarity(
                images_lbp, image_masks[im_i],
                self.pairwise_size, self.pairwise_dilation
            )
            # print("LBP") 
            # print(images_color_similarity_lbp)
            """
            HOG
            """
            # images_gray = color.rgb2gray(downsampled_images[im_i].byte().permute(1, 2, 0).cpu().numpy())
            # fd, images_hog = skft.hog(images_gray, orientations=9, pixels_per_cell=(8, 8),
            #         cells_per_block=(2, 4), visualize=True)
            # images_hog = numpy.array([images_hog, images_hog, images_hog])  
            # images_hog = images_hog.transpose(1, 2, 0)
            # images_hog = torch.as_tensor(images_hog, device=downsampled_images.device, dtype=torch.float32)
            # images_hog = images_hog.permute(2, 0, 1)[None]       
            # images_color_similarity_hog = get_images_color_similarity(
            #     images_hog, image_masks[im_i],
            #     self.pairwise_size, self.pairwise_dilation
            # ) 
            # print("HOG") 
            # print(images_color_similarity_hog)
            """
            YUV
            """
            # images_yuv = color.rgb2yuv(downsampled_images[im_i].byte().permute(1, 2, 0).cpu().numpy())
            # images_yuv = torch.as_tensor(images_yuv, device=downsampled_images.device, dtype=torch.float32)
            # images_yuv = images_yuv.permute(2, 0, 1)[None]
            # images_color_similarity_yuv = get_images_color_similarity(
            #     images_yuv, image_masks[im_i],
            #     self.pairwise_size, self.pairwise_dilation
            # )
            """
            新的计算方法
            ...
            # images_hog_similarity = get_images_color_similarity(
            #     images_hog, image_masks[im_i],
            #     self.pairwise_size, self.pairwise_dilation
            # )
            # images_lbp_similarity = get_images_color_similarity(
            #     images_lbp, image_masks[im_i],
            #     self.pairwise_size, self.pairwise_dilation
            # )

            # images_gray_similarity = get_images_color_similarity(
            #     images_gray, image_masks[im_i],
            #     self.pairwise_size, self.pairwise_dilation
            # )

           
            # images_color_similarity = images_lab_similarity

            """
            新计算方法 取最大值
            """
            # images_lab_similarity = (images_lab_similarity >= images_lbp_similarity)*images_lab_similarity
            # images_lbp_similarity = (images_lbp_similarity > images_lab_similarity)*images_lbp_similarity
            # images_color_similarity = images_lab_similarity + images_lbp_similarity
            # images_color_similarity = torch.maximum(images_lab_similarity, images_lbp_similarity)
            """
            HOG先验
            """
            # images_hog_similarity = (images_hog_similarity >= self.pairwise_precolor_thresh)*images_hog_similarity
            # images_color_similarity = images_lab_similarity + images_hog_similarity
            """
            LAB先验
            """
            # images_lab_similarity = (images_lab_similarity >= self.pairwise_precolor_thresh)*images_lab_similarity
            # images_color_similarity = images_lab_similarity + images_lbp_similarity

            # images_lab_similarity = (images_color_similarity_lab >= self.pairwise_precolor_thresh)*images_color_similarity_lab
            # # images_lbp_similarity = (images_color_similarity_lab < self.pairwise_precolor_thresh)*images_color_similarity_lbp
            # images_hog_similarity = (images_color_similarity_lab < self.pairwise_precolor_thresh)*images_color_similarity_hog
            # images_color_similarity = images_lab_similarity + images_hog_similarity
            """
            GRAY先验
            """
            # images_gray_similarity = (images_gray_similarity >= self.pairwise_precolor_thresh)*images_gray_similarity
            # images_color_similarity = images_lab_similarity + images_gray_similarity
            """
            两种颜色空间赋权重
            """
            images_color_similarity = images_color_similarity_lab*(0.9) + images_color_similarity_lbp*(0.1)
            # images_color_similarity = images_color_similarity_lab
            ...
            # ], dim=0)
            per_im_gt_inst.image_color_similarity_1 = torch.cat([
                images_color_similarity for _ in range(len(per_im_gt_inst))
            ], dim=0)
            # per_im_gt_inst.image_color_similarity_2 = torch.cat([
            #     images_color_similarity_xyz for _ in range(len(per_im_gt_inst))
            # ], dim=0)           
